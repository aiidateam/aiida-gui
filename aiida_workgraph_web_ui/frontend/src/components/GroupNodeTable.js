...
function GroupNodeTable() {
  ...
  const fetchGridData = useCallback(() => {
    ...
    const url =
      `http://localhost:8000/api/groupnode-data?` +
  ...
  useEffect(() => {
    if (toDeleteItem !== null) {
      fetch(`http://localhost:8000/api/groupnode/delete/${toDeleteItem.pk}?dry_run=True`, { method: 'DELETE' })
        .then(r => r.json())
        .then(data => {
  ...
  const handleDeleteNode = (item) => {
    fetch(`http://localhost:8000/api/groupnode/delete/${item.pk}`, { method: 'DELETE' })
      .then(r => r.json())
      .then(data => {
        if (data.deleted) {
  ...
  const processRowUpdate = useCallback(
    async (newRow, oldRow) => {
      ...
      try {
        const r = await fetch(`http://localhost:8000/api/groupnode/${newRow.pk}`, {
          method: 'PUT',
  ...
  const columns = [
    {
      ...
      headerName: 'PK',
      width: 90,
      renderCell: params => <Link to={`/groupnode/${params.row.pk}`}>{params.value}</Link>,
    },
